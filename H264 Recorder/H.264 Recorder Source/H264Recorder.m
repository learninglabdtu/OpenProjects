...
-(BOOL) startRecording {
    if(![self isRecording] && [self isConnected]) {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:recordingDir isDirectory:&isDirectory] && [[NSFileManager defaultManager] isWritableFileAtPath:recordingDir]) {
            recording = [[FFMpegWrapper alloc] initWithArguments:@[@"-analyzeduration", @"1000000", @"-i", @"-", @"-s", [NSString stringWithFormat:@"%dx%d", width, height], @"-r", @"50", @"-acodec", @"aac", @"-b:a", @"192k", @"-strict", @"-2", @"-vcodec", @"copy", currentOutputFile, @"-y"] autoRestart:NO label:@"recording"];
...
-(void) updateTimers {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (![autoSnapshotTimeout isEqualToNumber:@0] && [[self autoSnapshot] isValid]) {
            ...
            [self setAutoSnapshot:[NSTimer scheduledTimerWithTimeInterval:timeToFire target:self selector:@selector(stopAutoSnapshots) userInfo:nil repeats:NO]];
