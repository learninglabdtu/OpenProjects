...
- (void)windowDidLoad
{
    ...
    if([[appDelegate preferences] valueForKey:@"outputFormat"]) {
        ...
        if (item) {
            [[self formatSelect] selectItem: item];
        }
    }
    
    if([[appDelegate preferences] valueForKey:@"transcodeResolution"]) {
        NSMenuItem* item = [[self transcodeResolution] itemWithTitle:[[appDelegate preferences] valueForKey:@"transcodeResolution"]];
        if(item) {
            [[self transcodeResolution] selectItem:item];
        }
    }
